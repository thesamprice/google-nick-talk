...
namespaceAliasDecl;
AST_MATCHER_P(NamespaceAliasDecl, nominatesNamespace,
...
public:
  ...
  void qualify(SourceLocation SL, SourceManager *SM) {
    ...
    errs() << ' ' << std::string(data, data + (std::min(strlen(data), 16ul)))
           << "\n";
    Replace->insert(Replacement(*SM, SL, 0, Name + "::"));
  }
  ...
  virtual void run(const ast_matchers::MatchFinder::MatchResult &Result) {
    ...
    if (const DeclRefExpr *DRE = Result.Nodes.getNodeAs<DeclRefExpr>("dre")) {
      ...
      if (DRE->hasQualifier())
        return;
      if (UsingList.count(DRE->getDecl()))
        return;
    ...
    if (const TypeLoc *TL = Result.Nodes.getNodeAs<TypeLoc>("type")) {
      ...
      if (UsingList.count(TTL.getDecl()))
        return;
      qualify(TTL.getNameLoc(), SM);
      return;
    }
    if (const UsingDecl *UD = Result.Nodes.getNodeAs<UsingDecl>("using")) {
      errs() << "using\n";
      for (auto shadowed : UD->shadows()) {
        UsingList.insert(shadowed->getTargetDecl());
      }
      return;
    }
    if (const NamespaceAliasDecl *NAD =
            Result.Nodes.getNodeAs<NamespaceAliasDecl>("alias")) {
      errs() << "alias\n";
      Name = NAD->getNameAsString();
      return;
    }
...
private:
  ...
  std::set<const NamedDecl *> UsingList;
  std::string Name;
};
...
int main(int argc, const char **argv) {
  ...
  Finder.addMatcher(usingDecl().bind("using"), &Callback);
  Finder.addMatcher(
      namespaceAliasDecl(nominatesNamespace(ns_name)).bind("alias"), &Callback);
  Finder.addMatcher(
